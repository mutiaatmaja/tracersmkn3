                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        @if ($isAlumniUser)
                            ...
                            <p class="text-gray-600">Ringkasan tracer study milik Anda.</p>
                        @else
            ...
            @if ($isAlumniUser)
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                    <div class="bg-white rounded-xl shadow-md p-6">
                        <p class="text-sm text-gray-500">Status Tracer Tahun Ini</p>
                        <p class="text-2xl font-bold text-gray-900 mt-2">
                            @if ($alumniTracerTahunIniStatus === 'sudah_isi')
                                Sudah Isi
                            @elseif ($alumniTracerTahunIniStatus === 'draft')
                                Draft
                            @else
                                Belum Isi
                            @endif
                        </p>
                        <p class="text-xs text-gray-500 mt-2">Periode {{ now()->year }}</p>
                    </div>

                    <div class="bg-white rounded-xl shadow-md p-6">
                        <p class="text-sm text-gray-500">Total Pengisian Tracer</p>
                        <p class="text-2xl font-bold text-gray-900 mt-2">{{ number_format($alumniTracerHistory->count()) }}
                        </p>
                        <p class="text-xs text-gray-500 mt-2">Riwayat data tracer milik Anda.</p>
                    </div>

                    <div class="bg-white rounded-xl shadow-md p-6">
                        <p class="text-sm text-gray-500">Terakhir Mengisi</p>
                        <p class="text-2xl font-bold text-gray-900 mt-2">
                            {{ optional($alumniTracerHistory->firstWhere('status', 'submitted')?->submitted_at)->format('d M Y') ?? '-' }}
                        </p>
                        <p class="text-xs text-gray-500 mt-2">Tanggal kirim final terakhir.</p>
                    </div>

                    <div class="bg-white rounded-xl shadow-md p-6">
                        <p class="text-sm text-gray-500">Bisa Isi Tracer Lagi</p>
                        <p class="text-2xl font-bold text-gray-900 mt-2">
                            {{ $alumniCanFillTracer ? 'Sekarang' : optional($alumniBisaIsiLagiPada)->format('d M Y') }}
                        </p>
                        <p class="text-xs text-gray-500 mt-2">Jadwal berikutnya untuk isi tracer.</p>
                    </div>
                </div>

                @if ($alumniTracerTahunIniStatus === 'sudah_isi')
                    <div class="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800">
                        Anda sudah mengisi tracer tahun ini.
                    </div>
                @elseif ($alumniTracerTahunIniStatus === 'draft')
                    <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                        Anda masih memiliki draft tracer tahun ini. Silakan lanjutkan hingga kirim final.
                    </div>
                @else
                    <div class="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
                        Anda belum mengisi tracer tahun ini.
                    </div>
                @endif

                <div class="bg-white rounded-xl shadow-md p-6">
                    <h2 class="text-lg font-bold text-gray-900">Riwayat Pengisian Tracer Anda</h2>
                    <p class="text-sm text-gray-500 mt-1">Daftar tracer yang pernah Anda input.</p>
                    ...
                    <div class="mt-4 overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @forelse ($alumniTracerHistory as $submission)
                                    <tr>
                                        <td class="px-4 py-3 text-sm text-gray-800">{{ $submission->periode_tahun }}</td>
                                        <td class="px-4 py-3 text-sm">
                                            <span
                                                class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $submission->status === 'submitted' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700' }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-4 py-6 text-center text-sm text-gray-500">Belum
                                            ada riwayat tracer.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
